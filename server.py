...
#스케쥴 테이블 설정 좀 하자
#1. gui시작되면 일단 디폴트로 시간이 들어 가게하자. 지금 처럼 범위 설정으로 랜덤하게 들어가게
#2. 수동으로도 시간을 설정 할 수 있게 하자
#3. 스케쥴 설정 버튼을 눌러서 설정이 되면 스케쥴 설정 버튼이 초록색으로 변하게 하자 아닐때는 빨간색
#4. 23시 55분에 초기화 되고 재설정되게 하자.
